//! Tier-A validation of the CSC paths (`("no"|"bbox"|"full", "csc", "cython")`,
//! 1D + 2D). The built CSC matrix (transpose of the CSR LUT) is validated against
//! the golden `csc_data`/`csc_indices`/`csc_indptr`, and the applied output
//! (`csc_integrate1d`/`csc_integrate2d`, fed the *golden* CSC + *golden* preproc
//! rows so a build failure can't mask an apply failure) against the golden
//! `out_*`. Every field is asserted bit-exact: the build matches scipy's
//! `tocsc`, and the pixel-major scatter reproduces pyFAI's `CscIntegrator.
//! integrate_ng` bit-for-bit.
...
use rsfai_integrate::{
    ...
    csc_integrate2d, Bbox2dBounds, Csc,
};
...
/// Does this dataset's method tuple match `(split, "csc")` at the given dim?
fn is_csc(cfg: &serde_json::Value, split: &str, dim: u64) -> bool {
    let matches = cfg["method"]
        .as_array()
        .map(|m| m.len() >= 3 && m[0].as_str() == Some(split) && m[1].as_str() == Some("csc"))
        .unwrap_or(false);
    matches && cfg["dim"].as_u64().unwrap_or(1) == dim
}

/// Compare the built CSC matrix against the golden, then apply the *golden* CSC +
...
fn validate_csc_1d(
    ...
    assert_eq!(unit, "q_nm^-1", "{dataset}: 1D CSC golden assumes q_nm^-1");
    ...
    let g_data = vec_f32(dir, "csc_data.npy");
    let g_indices = vec_i32(dir, "csc_indices.npy");
    let g_indptr = vec_i32(dir, "csc_indptr.npy");

    let r_data = compare_f32(&built.data, &g_data);
    let indices_eq = built.indices == g_indices;
    let indptr_eq = built.indptr == g_indptr;
    eprintln!(
        "{dataset}: csc nnz={} (golden {}) | data mism={} | indices eq={} | indptr eq={}",
        built.data.len(),
        g_data.len(),
    ...
    assert!(indptr_eq, "{dataset}: csc_indptr mismatch");
    assert!(indices_eq, "{dataset}: csc_indices mismatch");
    assert!(
        r_data.is_bit_exact(),
        "{dataset}: csc_data not bit-exact: {r_data:?}"
    );
    ...
    let out = csc_integrate1d(&golden, &prep, bin_centers, error_model, 0.0);
    ...
    eprintln!("{dataset}: 1D CSC build + apply all fields bit-exact");
}

/// Compare the built 2D CSC matrix against the golden, then apply the *golden*
/// CSC + *golden* preproc and assert every 2D output field bit-exact.
fn validate_csc_2d(
    dir: &std::path::Path,
    dataset: &str,
    cfg: &serde_json::Value,
    ...
    let indptr_eq = built.indptr == g_indptr;
    eprintln!(
        "{dataset}: 2D csc nnz={} (golden {}) | data mism={} | indices eq={} | indptr eq={}",
        built.data.len(),
        g_data.len(),
        r_data.bit_mismatches,
        indices_eq,
    ...
    let prep = vec_f32(dir, "preproc.npy");
    let out = csc_integrate2d(&golden, &prep, bin_centers0, bin_centers1, error_model, 0.0);
    ...
    eprintln!("{dataset}: 2D CSC build + apply all fields bit-exact");
}

#[test]
fn nosplit_csc_build_and_apply_bit_exact() {
    ...
    assert!(
        checked > 0,
        "no no-split-csc 1D golden datasets found; run golden/gen_golden.py"
    );
}

#[test]
fn bbox_csc_build_and_apply_bit_exact() {
    let mut checked = 0usize;
    for dir in dataset_dirs() {
        let manifest = load_manifest(dir.join("manifest.json")).expect("manifest");
        let cfg = &manifest.config;
        if !is_csc(cfg, "bbox", 1) {
            continue;
        }
        let npt = cfg["npt"].as_u64().expect("npt") as usize;
    ...
    assert!(
        checked > 0,
        "no bbox-csc 1D golden datasets found; run golden/gen_golden.py"
    );
}

#[test]
fn full_csc_build_and_apply_bit_exact() {
    let mut checked = 0usize;
    for dir in dataset_dirs() {
        let manifest = load_manifest(dir.join("manifest.json")).expect("manifest");
        let cfg = &manifest.config;
        if !is_csc(cfg, "full", 1) {
            continue;
        }
        let npt = cfg["npt"].as_u64().expect("npt") as usize;
        // 1D full does not forward chiDiscAtPi/pos1_period -> defaults True, 2π.
        let (built, bin_centers) = build_full_csc_1d(
            &corners_f64(&dir),
            Some(&mask(&dir)),
            npt,
            false,
    ...
    assert!(
        checked > 0,
        "no full-csc 1D golden datasets found; run golden/gen_golden.py"
    );
}

#[test]
fn nosplit_csc_2d_build_and_apply_bit_exact() {
    let mut checked = 0usize;
    for dir in dataset_dirs() {
        let manifest = load_manifest(dir.join("manifest.json")).expect("manifest");
        let cfg = &manifest.config;
    ...
    assert!(
        checked > 0,
        "no no-split-csc 2D golden datasets found; run golden/gen_golden.py"
    );
}

#[test]
fn bbox_csc_2d_build_and_apply_bit_exact() {
    let mut checked = 0usize;
    for dir in dataset_dirs() {
        let manifest = load_manifest(dir.join("manifest.json")).expect("manifest");
        let cfg = &manifest.config;
        if !is_csc(cfg, "bbox", 2) {
            continue;
        }
        let npt_rad = cfg["npt_rad"].as_u64().expect("npt_rad") as usize;
        let npt_azim = cfg["npt_azim"].as_u64().expect("npt_azim") as usize;
    ...
    assert!(
        checked > 0,
        "no bbox-csc 2D golden datasets found; run golden/gen_golden.py"
    );
}

#[test]
fn full_csc_2d_build_and_apply_bit_exact() {
    let mut checked = 0usize;
    for dir in dataset_dirs() {
        let manifest = load_manifest(dir.join("manifest.json")).expect("manifest");
        let cfg = &manifest.config;
        if !is_csc(cfg, "full", 2) {
            continue;
        }
        let npt_rad = cfg["npt_rad"].as_u64().expect("npt_rad") as usize;
        let npt_azim = cfg["npt_azim"].as_u64().expect("npt_azim") as usize;
    ...
    assert!(
        checked > 0,
        "no full-csc 2D golden datasets found; run golden/gen_golden.py"
    );
}
